...
#define TELEINFO_DEVICE 	"/dev/ttyUSB0"
#define TELEINFO_PORT			1200							// Port used to send frame over Network
#define TELEINFO_NETWORK	"192.168.0.255"		// Broadcast or spcific IP where to send frame
...
#define MYSQL_LOGIN "open2300"
#define MYSQL_PASS	"mysql2300"
#define MYSQL_DB 		"open2300"
...
char 	g_lockname[_POSIX_PATH_MAX] = ""; // Lock filename
...
void clean_exit (int exit_code)
{
  ...
  if (g_fd_teleinfo)
  {
	...
	if ( exit_code == EXIT_SUCCESS)
		log_syslog(stdout, "Closing teleinfo\n");
...
int tlf_init_serial(void)
{
  ...
  if (  (r = tcgetattr(tty_fd, &g_oldtermios)) < 0 )
	...
	//	fatal("cannot set term speed %s: %s", opts.port, strerror(errno));
